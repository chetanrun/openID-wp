...
/**
 * OpenID_Wp_Client_Wrapper class.
 *
 ...
 * @category Authentication
 */
class OpenID_Wp_Client_Wrapper {
	 ...
	 * @var string
	 */
	private $cookie_token_refresh_key = 'openID-wp-refresh';
	 ...
	 * @var string
	 */
	public $cookie_redirect_key = 'openID-wp-redirect';
	 ...
	 *
	 * @param OpenID_Wp_Client          $client   A plugin client object instance.
	 ...
	 */
	public function __construct( OpenID_Wp_Client $client, OpenID_Wp_Option_Settings $settings, OpenID_Wp_Option_Logger $logger ) {
		$this->client = $client;
	 ...
	 *
	 * @param \OpenID_Wp_Client          $client   The plugin client instance.
	 * @param \OpenID_Wp_Option_Settings $settings The plugin settings instance.
	 * @param \OpenID_Wp_Option_Logger   $logger   The plugin logger instance.
	 *
	 * @return \OpenID_Wp_Client_Wrapper
	 */
	public static function register( OpenID_Wp_Client $client, OpenID_Wp_Option_Settings $settings, OpenID_Wp_Option_Logger $logger ) {
		...
		add_filter( 'openID-wp-alter-request', array( $client_wrapper, 'alter_request' ), 10, 3 );
	...
	public function get_redirect_to() {
		...
		$redirect_url = apply_filters_deprecated(
			'openID-wp-cookie-redirect-url',
			array( $redirect_url ),
			'3.8.2',
			'openID-wp-client-redirect-to'
		);

		// This is the new hook to use with the transients version of redirection.
		return apply_filters( 'openID-wp-client-redirect-to', $redirect_url );
	}
	...
	public function get_authentication_url( $atts = array() ) {
		...
		$this->logger->log( apply_filters( 'openID-wp-auth-url', $url ), 'make_authentication_url' );
		return apply_filters( 'openID-wp-auth-url', $url );
	}
	...
	public function ensure_tokens_still_fresh() {
		...
		if ( ! $refresh_token || ( $refresh_expires && $current_time > $refresh_expires ) ) {
			...
			if ( $this->settings->redirect_on_logout ) {
				$this->error_redirect( new WP_Error( 'access-token-expired', __( 'Session expired. Please login again.', 'unifyia-openID-wp' ) ) );
			}
	...
	public function get_end_session_logout_redirect_url( $redirect_url, $requested_redirect_to, $user ) {
		...
		$token_response = $user->get( 'openID-wp-last-token-response' );
		if ( ! $token_response ) {
		...
		$claim = $user->get( 'openID-wp-last-id-token-claim' );
	...
	public function authentication_request_callback() {
		...
		if ( ! $user ) {
			if ( $this->settings->create_if_does_not_exist ) {
				...
				$this->error_redirect( new WP_Error( 'identity-not-map-existing-user', __( 'User identity is not linked to an existing WordPress user.', 'unifyia-openID-wp' ), $user_claim ) );
			}
		} else {
			// Allow plugins / themes to take action using current claims on existing user (e.g. update role).
			do_action( 'openID-wp-update-user-using-current-claim', $user, $user_claim );
		}
		...
		do_action( 'openID-wp-user-logged-in', $user );
		...
		$state_object = get_transient( 'openID-wp-state--' . $state );
		...
		if ( $this->settings->redirect_user_back ) {
			do_action( 'openID-wp-redirect-user-back', $redirect_url, $user );
		}
	...
	public function validate_user( $user ) {
		...
		if ( ! is_a( $user, 'WP_User' ) || ! $user->exists() ) {
			return new WP_Error( 'invalid-user', __( 'Invalid user.', 'unifyia-openID-wp' ), $user );
		}
	...
	public function login_user( $user, $token_response, $id_token_claim, $user_claim, $subject_identity ) {
		...
		update_user_meta( $user->ID, 'openID-wp-last-token-response', $token_response );
		update_user_meta( $user->ID, 'openID-wp-last-id-token-claim', $id_token_claim );
		update_user_meta( $user->ID, 'openID-wp-last-user-claim', $user_claim );
	...
	public function get_user_by_identity( $subject_identity ) {
		// Look for user by their openID-wp-subject-identity value.
		$user_query = new WP_User_Query(
			array(
				'meta_query' => array(
					array(
						'key'   => 'openID-wp-subject-identity',
	...
	private function get_username_from_claim( $user_claim ) {
		...
		if ( ! empty( $this->settings->identity_key ) && isset( $user_claim[ $this->settings->identity_key ] ) ) {
			...
			return new WP_Error( 'no-username', __( 'No appropriate username found.', 'unifyia-openID-wp' ), $user_claim );
		}
		...
		if ( empty( $transliterated_username ) ) {
			...
			return new WP_Error( 'username-transliteration-failed', sprintf( __( 'Username %1$s could not be transliterated.', 'unifyia-openID-wp' ), $desired_username ), $desired_username );
		}
		...
		if ( empty( $normalized_username ) ) {
			...
			return new WP_Error( 'username-normalization-failed', sprintf( __( 'Username %1$s could not be normalized.', 'unifyia-openID-wp' ), $transliterated_username ), $transliterated_username );
		}
	...
	private function get_nickname_from_claim( $user_claim ) {
		...
		if ( empty( $desired_nickname ) ) {
			...
			return new WP_Error( 'no-nickname', sprintf( __( 'No nickname found in user claim using key: %1$s.', 'unifyia-openID-wp' ), $this->settings->nickname_key ), $this->settings->nickname_key );
		}
				...
				if ( ! isset( $user_claim[ $key ] ) ) {
					if ( $error_on_missing_key ) {
						return new WP_Error(
							...
							__( 'User claim incomplete.', 'unifyia-openID-wp' ),
							array(
	...
	public function create_new_user( $subject_identity, $user_claim ) {
		$user_claim = apply_filters( 'openID-wp-alter-user-claim', $user_claim );
		...
		if ( $values_missing && isset( $user_claim['access_token'] ) && ! empty( $this->settings->endpoint_userinfo ) ) {
			...
			if ( is_wp_error( $user_claim_result ) ) {
				return new WP_Error( 'bad-user-claim-result', __( 'Bad user claim result.', 'unifyia-openID-wp' ), $user_claim_result );
			}
		 ...
		 */
		$create_user = apply_filters( 'openID-wp-user-creation-test', true, $user_claim );

		if ( ! $create_user ) {
			return new WP_Error( 'cannot-authorize', __( 'Can not authorize.', 'unifyia-openID-wp' ), $create_user );
		}
		...
		$user_data = apply_filters( 'openID-wp-alter-user-data', $user_data, $user_claim );
		...
		if ( is_wp_error( $uid ) ) {
			return new WP_Error( 'failed-user-creation', __( 'Failed user creation.', 'unifyia-openID-wp' ), $uid );
		}
		...
		add_user_meta( $user->ID, 'openID-wp-subject-identity', (string) $subject_identity, true );

		// Log the results.
		$this->logger->log( "New user created: {$user->user_login} ($uid)", 'success' );

		// Allow plugins / themes to take action on new user creation.
		do_action( 'openID-wp-user-create', $user, $user_claim );
	...
	public function update_existing_user( $uid, $subject_identity ) {
		...
		update_user_meta( $uid, 'openID-wp-subject-identity', strval( $subject_identity ) );

		// Allow plugins / themes to take action on user update.
		do_action( 'openID-wp-user-update', $uid );
