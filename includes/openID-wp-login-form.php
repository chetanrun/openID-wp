...
class OpenID_Wp_Login_Form {
	 ...
	 * @param OpenID_Wp_Client_Wrapper  $client_wrapper A plugin client wrapper object instance.
	 */
	...
	/**
	 * Create an instance of the OpenID_Wp_Login_Form class.
	 *
	 * @param OpenID_Wp_Option_Settings $settings       A plugin settings object instance.
	 * @param OpenID_Wp_Client_Wrapper  $client_wrapper A plugin client wrapper object instance.
	 *
	...
	public static function register( $settings, $client_wrapper ) {
		...
		add_shortcode( 'OpenID_Wp_login_button', array( $login_form, 'make_login_button' ) );
	...
	public function make_error_output( $error_code, $error_message ) {
		...
		<div id="login_error"><?php // translators: %1$s is the error code from the IDP. ?>
			<strong><?php printf( esc_html__( 'ERROR (%1$s)', 'unifyia-openID-wp' ), esc_html( $error_code ) ); ?>: </strong>
	...
	public function make_login_button( $atts = array() ) {
		...
		$atts = shortcode_atts(
			array(
				'button_text' => __( 'Login with OpenID Connect', 'unifyia-openID-wp' ),
			),
			$atts,
			'OpenID_Wp_login_button'
		);

		$text = apply_filters( 'openID-wp-login-button-text', $atts['button_text'] );
		$text = esc_html( $text );
